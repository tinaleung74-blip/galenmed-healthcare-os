...
import {
  RadiologyAuditActionBadge,
  RadiologyAuditCategoryBadge,
} from "@/features/radiology/components/radiology-audit-badges"
import {
  DEFAULT_RADIOLOGY_AUDIT_FILTERS,
  RADIOLOGY_AUDIT_ACTION_LABELS,
  RADIOLOGY_AUDIT_CATEGORY_LABELS,
  RADIOLOGY_AUDIT_INITIAL_VISIBLE_EVENTS,
} from "@/features/radiology/constants/radiology-audit.constants"
import {
  useRadiologyReports,
} from "@/features/radiology/providers/radiology-report-provider"
import {
  RADIOLOGY_AUDIT_EVENT_ACTIONS,
  RADIOLOGY_AUDIT_EVENT_CATEGORIES,
  type RadiologyAuditEvent,
  type RadiologyAuditEventAction,
  type RadiologyAuditEventCategory,
  type RadiologyAuditEventDetail,
  type RadiologyAuditFilters,
} from "@/features/radiology/types/radiology-audit.types"
import type {
  RadiologyOrder,
} from "@/features/radiology/types/radiology.types"
import {
  buildRadiologyAuditEvents,
} from "@/features/radiology/utils/radiology-audit.utils"
import {
  formatPatientDateTime,
  normalizePatientSearch,
} from "@/features/patients/utils/patient.utils"

interface RadiologyAuditHistoryProps {
...
function isCategoryFilter(
  ...
  return (
    value === "all" ||
    RADIOLOGY_AUDIT_EVENT_CATEGORIES.some(
      (category) =>
        category === value
    )
  )
...
function isActionFilter(
  ...
  return (
    value === "all" ||
    RADIOLOGY_AUDIT_EVENT_ACTIONS.some(
      (action) =>
        action === value
    )
  )
...
function protectSensitiveValue(
  ...
  return "Protected radiology content"
}

function matchesAuditSearch(
  event: RadiologyAuditEvent,
  search: string
): boolean {
...
export function RadiologyAuditHistory({
  order,
}: RadiologyAuditHistoryProps) {
  const { reports } =
    useRadiologyReports()

  const [filters, setFilters] =
    useState<RadiologyAuditFilters>(
      () => ({
        ...DEFAULT_RADIOLOGY_AUDIT_FILTERS,
      })
    )
    ...
    <section className="space-y-4">
        ...
        <div className="flex flex-col gap-2 xl:flex-row">
          <div className="relative min-w-0 flex-1">
            ...
            <Input
              ...
              placeholder="Search Radiology Audit events or actors"
              className="pl-8"
              onChange={(event) =>
                setFilters(
                  (
          ...
          >
            <option value="all">
              All radiology modules
            </option>

            {RADIOLOGY_AUDIT_EVENT_CATEGORIES.map(
              (category) => (
                <option
                  key={category}
                  value={category}
          ...
          >
            ...
            {RADIOLOGY_AUDIT_EVENT_ACTIONS.map(
              (action) => (
                <option
                  key={action}
                  value={action}
      ...
      <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800">
        ...
        <p>
          ...
          derived from Radiology Order,
          Preparation, Imaging, and Report
          timestamps. Production audit
